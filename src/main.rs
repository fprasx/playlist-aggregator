...
async fn main() {
    ...
    let new_playlist_id = match create_playlist(
        ...
        Some(false),
        Some(false),
        Some(&format!("Generated by Rust on {}", chrono::offset::Local::now())),
    )
    .await
    {
        Ok(id) => id,
...
// Get all tracks from liked songs
...
async fn get_tracks_from_playlists(spotify: &impl OAuthClient) -> HashSet<TrackId> {
    ...
    while let Some(Ok(playlist)) = playlists.next().await {
        ...
        total_duration = total_duration.checked_add(duration).unwrap();
        // TODO: Can't get padding right with brackets
        let bracketed_track = format!("<{}>", playlist.name);
        println!(
            "{CYAN}{:<40}{RESET} {GREEN}{:04}{RESET} tracks, {YELLOW}{}{RESET}",
            bracketed_track,
            num_tracks,
